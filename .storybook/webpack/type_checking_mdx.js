...
const delayTypeCheckingPlugin = function (options /*: { delay?: number } */) {
  return function () {
    ForkTsCheckerWebpackPlugin.getCompilerHooks(this).serviceBeforeStart.tapAsync(
      ...
      cb => {
        setTimeout(() => { cb() }, options.delay || 5000)
      }
    )
  }
}
...
module.exports = function (webpackConfig, options /*: { delay?: number } */) {
  ...
  const mdxRule = webpackConfig.module.rules.find(rule =>
    rule.test && typeof rule.test.test === 'function' && rule.test.test('.stories.mdx') && !rule.exclude
  )
  ...
  const forkTsCheckerWebpackPlugin = webpackConfig.plugins.find(p => p.constructor.name === 'ForkTsCheckerWebpackPlugin')
